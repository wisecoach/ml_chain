...
- [ ] trainer：任务链节点
  ...
  - [x] role：定义角色管理器，用于选取和管理网络中各个节点的角色
  - [x] trainer.go：trainer对象本身
  - [ ] model：
    - [x] aggregate：聚合模块
      - [x] 处理收到的局部模型
      - [x] 聚合模型
      - [x] 生成一轮迭代的交易，发送给挖矿者（就是自己吧，但是依旧推荐走comm，而不是内部依赖）
    - [x] train：训练模块
      - [x] 配置一个txhandler，将新一轮迭代的交易发送给该模块
      - [x] 发送模型验证请求和处理模型验证响应
      - [x] 管理训练任务
    - [x] validate：验证模块
      - [x] 处理模型验证请求，验证模型
